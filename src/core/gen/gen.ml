...
let () =
  let maj, min = Scanf.sscanf Sys.ocaml_version "%d.%d.%s" (fun x y _ -> x,y) in
  print_endline (if (maj,min) >= (4,12) then shims_atomic_after_412 else shims_atomic_before_412);
  ()
