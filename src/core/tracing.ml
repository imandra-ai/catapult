...
let[@inline never] emit_real_ (module B : BACKEND) ?ts_us ?cat ?(pid = pid)
...
let[@inline] emit ?stack ?ts_us ?cat ?pid ?tid ?args name ?dur
    (ev : Event_type.t) : unit =
  match !out_ with
  | None -> ()
...
let[@inline] instant ?stack ?ts_us ?cat ?pid ?tid ?args name =
  match !out_ with
  | None -> ()
  | Some b -> emit_real_ b ?stack ?ts_us ?cat ?pid ?tid ?args name I
...
let[@inline] meta ?ts_us ?cat ?pid ?tid ?args name =
  match !out_ with
  | None -> ()
  | Some b -> emit_real_ b ?ts_us ?cat ?pid ?tid name ?args M

let meta_thread_name name = meta "thread_name" ~args:[ "name", `String name ]
let meta_process_name name = meta "process_name" ~args:[ "name", `String name ]
...
let exit_with_ b ?stack ?cat ?pid ?tid ?args name start : unit =
  ...
  emit_real_ b ?cat ?pid ?tid ?args name ?stack ~ts_us:start ~dur X
...
let[@inline] obj_snap ?stack ?ts_us ?cat ?pid ?tid ?(args = []) name ~snapshot
    ~id =
  match !out_ with
  | None -> ()
  | Some b ->
    emit_real_ b ?stack ?ts_us ?cat ?pid ?tid name
      ~args:(("snapshot", `String snapshot) :: args)
      ~id O
...
let[@inline] a_begin ?ts_us ?pid ?tid ?args name ~cat ~id =
  match !out_ with
  | None -> ()
...
let[@inline] f_begin ?ts_us ?cat ?pid ?tid ?args name ~id =
  match !out_ with
  | None -> ()
  | Some b -> emit_real_ b ?ts_us ?cat ?pid ?tid ?args name ~id F_s

let[@inline] f_exit ?ts_us ?cat ?pid ?tid ?args name ~id =
  match !out_ with
  | None -> ()
  | Some b ->
    emit_real_ b ?ts_us ?cat ?pid ?tid ?args name ~id F_f ~extra:[ "bp", "e" ]

let[@inline] f_step ?ts_us ?cat ?pid ?tid ?args name ~id =
  match !out_ with
  | None -> ()
  | Some b -> emit_real_ b ?ts_us ?cat ?pid ?tid ?args name ~id F_t
