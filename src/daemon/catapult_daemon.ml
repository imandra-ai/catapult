...
module Writer : sig
  ...
  let close self =
    let@ () = with_lock self in
    let close_conn_ trace_id db = close_ self ~trace_id db in
  ...
  let create ~dir () : t =
    let self = { dir; dbs = Str_tbl.create 32; buf = Buffer.create 1024 } in
    self
end
...
module Server : sig
  ...
  let create ?(addr = Addr.default) ~writer () : t =
    ...
    let self = { writer; ctx; sock; addr; stop = Atomic.make false } in
    self
...
let () =
  ...
  in
  Arg.parse opts
    (fun _ -> raise (Arg.Help "no such option"))
    "catapult tracing daemon";
